...
class _ViewRoommateBookingScreenController extends LoadingController {
  Future<void> acceptBooking(RoommateBooking booking) async {
    ...
    try {
      ...
      final res = await ApiService.getDio
          .post("/bookings/roommate-ad/${booking.id}/offer");
  ...
  Future<void> cancelBooking(RoommateBooking booking) async {
    final shouldContinue = await showConfirmDialog(
    ...
    try {
      isLoading(true);
      final res = await ApiService.getDio
          .post("/bookings/roommate-ad/${booking.id}/cancel");
  ...
  void payRent(RoommateBooking booking) {
    Get.to(() => PayrentScreen(type: "ROOMMATE", ad: booking));
  }
}

class ViewRoommateBookingScreen extends StatelessWidget {
  const ViewRoommateBookingScreen({super.key, required this.booking});
  final RoommateBooking booking;

  @override
  Widget build(BuildContext context) {
    final controller = Get.put(_ViewRoommateBookingScreenController());
    return Scaffold(
        ...
        return Stack(
          children: [
            SingleChildScrollView(
              child: Column(
                children: [
                  RoommateBookingWidget(booking: booking),
                      ...
                      child: SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: controller.isLoading.isTrue
                              ? null
                              : () async {
                                  final conv = (await ChatConversation
                                          .getSavedChat(
